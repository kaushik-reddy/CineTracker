...
import { X, Plus, Film, Tv, Book, Sparkles, CheckCircle, Loader2 } from "lucide-react";
import { supabaseAdapter as base44 } from '@/api/supabaseAdapter';
import { fetchMediaDetails } from '@/api/openaiClient';
...
export default function MediaForm({ open, onClose, onSubmit, initialData }) {
  ...
  const applyPreviewData = () => {
    ...
    setFormData(prev => ({
      ...prev,
      ...previewData,
      // Ensure specific fields map correctly if JSON schema differs slightly
      runtime_minutes: previewData.runtime_minutes?.toString() || prev.runtime_minutes,
      total_pages: previewData.total_pages?.toString() || prev.total_pages,
      year: previewData.year?.toString() || prev.year,
      seasons_count: previewData.seasons_count?.toString() || prev.seasons_count,
    }));

    // Also update inputs that track array/string state if needed
    if (previewData.actors) setActorInput('');
