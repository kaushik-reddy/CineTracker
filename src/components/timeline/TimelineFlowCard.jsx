...
import { Calendar, Clock, CheckCircle, RotateCcw, AlertCircle, Play, Pause, Book, Tv, Film, Users } from "lucide-react";
...
export default function TimelineFlowCard({
  ...
  const scheduledTime = safeDate(schedule.scheduled_date) || new Date();
              ...
              <div className="flex-1 min-w-0">
                ...
                {media.platform && (
                  <div className="mt-1 flex items-center gap-1 flex-wrap">
                    ...
                    {schedule.audio_format && <AudioFormatBadge format={schedule.audio_format} size="small" />}
                    {schedule.video_format && <VideoFormatBadge format={schedule.video_format} size="small" />}
                  </div>
                )}
                {isDelayed && (
            ...
            <div className="flex-1 min-w-0">
              <div className="relative flex items-center justify-between mb-2 px-2">
                ...
                <div className="flex flex-col items-center z-10 flex-1">
                  <div className={`w-8 h-8 lg:w-10 lg:h-10 rounded-full flex items-center justify-center border-2 transition-all ${isScheduled || isStarted || isCompleted ? 'bg-gradient-to-br from-purple-500 to-emerald-500 border-purple-500 shadow-lg' : 'bg-zinc-800 border-zinc-600'}`}>
                    <Calendar className={`w-4 h-4 lg:w-5 lg:h-5 ${isScheduled || isStarted || isCompleted ? 'text-white' : 'text-zinc-500'}`} />
                  </div>
                  <span className={`text-[10px] lg:text-xs font-semibold mt-1 ${isScheduled || isStarted || isCompleted ? 'text-white' : 'text-zinc-500'}`}>Scheduled</span>
                  <div className="mt-2 text-center">
                    <p className="text-zinc-400 text-[9px]">Scheduled Time</p>
                    <p className="text-white font-medium text-[10px]">{safeFormat(scheduledTime, 'MMM d, h:mm a')}</p>
                  </div>
                </div>

                <div className={`flex-1 h-0.5 mx-1 transition-all ${isStarted || isCompleted ? 'bg-gradient-to-r from-purple-500 to-emerald-500' : 'bg-zinc-700'}`} />

                {/* Started */}
                <div className="flex flex-col items-center z-10 flex-1">
                  <div className={`w-8 h-8 lg:w-10 lg:h-10 rounded-full flex items-center justify-center border-2 transition-all ${schedule.status === 'in_progress' ? 'bg-gradient-to-br from-emerald-500 to-emerald-400 border-emerald-500 shadow-lg' : schedule.status === 'paused' ? 'bg-gradient-to-br from-amber-500 to-amber-400 border-amber-500 shadow-lg' : isStarted || isCompleted ? 'bg-gradient-to-br from-emerald-500 to-amber-500 border-emerald-500 shadow-lg' : 'bg-zinc-800 border-zinc-600'}`}>
                    {schedule.status === 'paused' ? <Pause className="w-4 h-4 lg:w-5 lg:h-5 text-white" /> : <Play className={`w-4 h-4 lg:w-5 lg:h-5 ${isStarted || isCompleted ? 'text-white' : 'text-zinc-500'}`} />}
                  </div>
                  <span className={`text-[10px] lg:text-xs font-semibold mt-1 whitespace-nowrap ${schedule.status === 'in_progress' ? 'text-emerald-400' : schedule.status === 'paused' ? 'text-amber-400' : isStarted || isCompleted ? 'text-white' : 'text-zinc-500'}`}>
                    {schedule.status === 'in_progress' ? 'Started · Playing' : schedule.status === 'paused' ? 'Paused' : 'Started'}
                  </span>
                  {startedTime && (
                    <div className="mt-2 text-center">
                      <p className="text-zinc-400 text-[9px]">Start Time</p>
                      <p className="text-emerald-400 font-medium text-[10px]">{safeFormat(startedTime, 'MMM d, h:mm a')}</p>
                    </div>
                  )}
                </div>

                <div className={`flex-1 h-0.5 mx-1 transition-all ${isCompleted ? 'bg-gradient-to-r from-emerald-500 to-blue-500' : 'bg-zinc-700'}`} />

                {/* Completed */}
                <div className="flex flex-col items-center z-10 flex-1">
                  <div className={`w-8 h-8 lg:w-10 lg:h-10 rounded-full flex items-center justify-center border-2 transition-all ${isCompleted ? 'bg-gradient-to-br from-blue-500 to-purple-500 border-blue-500 shadow-lg' : 'bg-zinc-800 border-zinc-600'}`}>
                    <CheckCircle className={`w-4 h-4 lg:w-5 lg:h-5 ${isCompleted ? 'text-white' : 'text-zinc-500'}`} />
                  </div>
                  <span className={`text-[10px] lg:text-xs font-semibold mt-1 ${isCompleted ? 'text-white' : 'text-zinc-500'}`}>Completed</span>
                  {completedTime && (
                    <div className="mt-2 text-center">
                      <p className="text-zinc-400 text-[9px]">End Time</p>
                      <p className="text-blue-400 font-medium text-[10px]">{safeFormat(completedTime, 'MMM d, h:mm a')}</p>
                    </div>
                  )}
                </div>
              </div>
                ...
                {totalTimeSpent !== null && (
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-white font-bold text-[10px]">{Math.floor(totalTimeSpent / 60)}h {totalTimeSpent % 60}m</p>
                    </div>
                    {timeOverrun !== null && timeOverrun > 0 && (
                      <div className="text-right">
                        <p className="text-zinc-400 text-[9px]">Overrun</p>
                        <p className="text-orange-400 font-medium text-[10px]">+{Math.floor(timeOverrun / 60)}h {timeOverrun % 60}m</p>
                      </div>
                    )}
                  </div>
                )}
                  ...
                  <div className={`${totalTimeSpent !== null ? 'pt-1.5 border-t border-zinc-700' : ''}`}>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-emerald-400 font-medium text-[10px]">{Math.floor(currentElapsed / 60)}m {currentElapsed % 60}s / {runtime}m</p>
                      </div>
                      <div className="text-right">
                        <p className="text-zinc-400 text-[9px]">Remaining</p>
                        <p className="text-white font-medium text-[10px]">{Math.max(0, Math.floor((runtime * 60 - currentElapsed) / 60))}m</p>
                      </div>
                    </div>
                  </div>
                )}
              ...
              {schedule.status !== 'completed' && (
                <div className="flex gap-1.5 mt-2">
                  {schedule.is_watch_party ? (
                    ...
                    >
                      <Users className="w-3 h-3 mr-1" /> Join Party
                    </Button>
                  ) : (
                    <Button
                      size="sm"
                      ...
                      className={`flex-1 text-[10px] px-2 py-1.5 h-auto ${!canMarkComplete ? 'bg-zinc-700 text-zinc-500 cursor-not-allowed opacity-50' : 'bg-emerald-500 hover:bg-emerald-600 text-white'}`}
                    >
                  ...
                  <Button
                    ...
                    className={`flex-1 text-[10px] px-2 py-1.5 h-auto ${!canEdit ? 'bg-zinc-700 text-zinc-500 cursor-not-allowed opacity-50 border-zinc-700' : 'bg-white hover:bg-zinc-100 text-black'}`}
                  >
